...
//    vec2 newVel10 = getNewVel(vel0, vel1, I0.r, I1.r, e1);
    ...
    float massInflow20 = getMassInflow(vel2, e2, I2.r);
    massInflow += massInflow20;
    vec2 newVel20 = getNewVel(vel0, vel2, I0.r, I2.r, e2);
    
    
    // DEBUG ------ 
    float sVel2 = weightSum(vel2);
    float massInflow20_DEBUG = 0.0;
    if (dot(vel2, -e2) > 0.0) {
        massInflow20_DEBUG = I2.r * dot(vel2, -e2) / sVel2 / offsetPx;
    }
    // DEBUG ------ 
...
//    float massInflow40 = getMassInflow(vel4, e4, I4.r);
...
//    vec2 newVel50 = getNewVel(vel0, vel5, I0.r, I5.r, e5);
    ...
    float massInflow60 = getMassInflow(vel6, e6, I6.r);
...
//    float massInflow80 = getMassInflow(vel8, e8, I8.r);
...
//                newVel40 + newVel50 + newVel60 + newVel70 + newVel80;
        ...
        newVel += newVel20 + newVel60;
    }


    vec4 finalColor = itc(vec3(newMass, newVel.x, newVel.y));

    // TODO : почему масса в центре постепенно стремится к 0.0 ?

//     if (massOutflow > massInflow && massOutflow - massInflow > 0.1) {
//         finalColor = vec4(1.0);
//     }

//    if (abs(massInflow20_DEBUG - w06 * I0.r) == 0.0) {
//        finalColor = vec4(1.0);
//    }
